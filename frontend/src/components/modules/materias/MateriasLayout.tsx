...
import { useMateriasTable } from './logic/useMateriasTable';
import { useMateriasActions } from './logic/useMateriasActions';
import { TOAST_ACCENT, TOAST_ICON_COLOR, TIPO_AULA_INFO } from './logic/constants';
import type { Materia, Toast } from './logic/types';
...
function TipoAulaBadge({ tipo }: { tipo: string | null }) {
  ...
  const info =
    TIPO_AULA_INFO[tipo.toLowerCase()] ??
    { label: tipo, className: 'bg-gray-100 text-gray-600 border border-gray-200', Icon: LayoutGrid };
...
export default function MateriasLayout() {
  const { materias, total, loading, error, searchTerm, setSearchTerm, filtered, fetchMaterias } =
    useMateriasTable();
  const { toasts, removeToast, handleSave, handleDelete, handleSuspend } = useMateriasActions(fetchMaterias);
  ...
  const openConfirm = (id: number | undefined) => {
    ...
    setConfirm({ open: true, id, msg: `¿Deseas eliminar permanentemente la materia "${materia?.nombre ?? id}"?` });
  };
  ...
  const confirmDelete = async () => {
    ...
    if (!id) return;
    await handleDelete(id, materia?.nombre);
  };
    ...
    <div className="p-4 sm:p-6 md:p-8 min-h-full bg-[#081028] text-white">
      ...
      {error && !loading && (
        <div className="flex items-start gap-3 bg-red-500/10 border border-red-500/20
          ...
          <div>
            <p className="font-semibold">No se pudieron cargar las materias</p>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((materia) => (
                  <tr key={materia.id} className="hover:bg-white/[0.03] transition-colors">
                    ...
                    <td className="px-4 py-3 text-white max-w-[200px] truncate text-sm" title={materia.nombre}>
                      {materia.nombre}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button
                          onClick={() => handleSuspend(materia.id, materia)}
                        ...
                        <button
                          onClick={() => openConfirm(materia.id)}
      ...
      <MateriasModal
        ...
        onSave={(materia) => {
          handleSave(materia, selectedMateria);
